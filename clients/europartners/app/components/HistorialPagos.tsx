'use client'
import { useState, useEffect } from 'react'
import { Receipt } from 'lucide-react'
import { formatUSD } from '@/lib/precio'
import type { Pago } from '@/types/europartners'

function fechaCorta(iso: string) {
  const d = new Date(iso)
  return d.toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' })
}

// Historial de pagos (Registro Maestro Vivo) de una proforma — cobros al
// cliente y pagos a China. Consume GET /api/proformas/[id]/pagos (ver
// migración 021_tabla_pagos.sql). Componente compartido entre el editor de
// proforma (cotizador/[id]) y el drawer de detalle de /finanzas, para no
// duplicar el fetch ni el render del timeline en dos lugares.
export default function HistorialPagos({
  proformaId,
  refreshKey = 0,
}: {
  proformaId: string
  refreshKey?: number
}) {
  const [pagos, setPagos] = useState<Pago[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    let cancelado = false
    setLoading(true)
    fetch(`/api/proformas/${proformaId}/pagos`)
      .then(r => r.json())
      .then(({ data }) => { if (!cancelado) setPagos(data || []) })
      .catch(() => { if (!cancelado) setPagos([]) })
      .finally(() => { if (!cancelado) setLoading(false) })
    return () => { cancelado = true }
  }, [proformaId, refreshKey])

  if (loading) {
    return <p className="text-sm text-gray-400">Cargando pagos...</p>
  }

  if (pagos.length === 0) {
    return <p className="text-sm text-gray-400">Todavía no hay pagos registrados para esta proforma.</p>
  }

  return (
    <div className="space-y-2">
      {pagos.map(pago => (
        <div key={pago.id} className="flex items-start justify-between gap-3 border-b border-gray-50 last:border-0 pb-2 last:pb-0">
          <div className="flex items-start gap-2.5">
            <div className={`mt-0.5 w-7 h-7 rounded-full flex items-center justify-center flex-none ${
              pago.tipo === 'cliente' ? 'bg-blue-50 text-blue-600' : 'bg-amber-50 text-amber-600'
            }`}>
              <Receipt size={13} />
            </div>
            <div>
              <p className="text-sm text-gray-700">
                <span className="font-semibold">{formatUSD(pago.monto)}</span>{' '}
                <span className="text-gray-400">— {pago.tipo === 'cliente' ? 'cobro al cliente' : 'pago a China'}</span>
              </p>
              <p className="text-xs text-gray-400 mt-0.5">
                {fechaCorta(pago.fecha)}
                {pago.referencia && ` · ${pago.referencia}`}
                {pago.comision_bancaria > 0 && ` · comisión ${formatUSD(pago.comision_bancaria)}`}
                {' · '}{pago.usuario?.nombre || 'Cliente (autoservicio)'}
              </p>
              {pago.nota && <p className="text-xs text-gray-400 mt-0.5">{pago.nota}</p>}
            </div>
          </div>
          {pago.comprobante_url && (
            <a
              href={pago.comprobante_url}
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs text-blue-600 underline flex-none"
            >
              Ver comprobante
            </a>
          )}
        </div>
      ))}
    </div>
  )
}
